...
<table border=1 class="reportTable">
	...
	<tr>
		<td>${rp.st_num}</td>
		<td>${rp.reportCnt}</td>
		<td><a href='/admin/reportDel?column=2&&number=${rp.st_num}'>삭제</a></td>
		<td><a href='/admin/reportCancel?column=2&&cancel_num=${rp.st_num}'>신고취소</a></td>
		<td><a href="/admin/storyReportDetail?st_num=${rp.st_num}" target="_blank">자세히</a></td>
	</tr>
	</c:forEach>	
</table>
...
<ul >
	...
	<c:if test="${pageMaker.prev }">	
		<li>
			<a href="/admin/ReportList?page=${pageMaker.startPage - 1 }&&column=2"></a>
		</li>
	</c:if>
	<!-- 각 페이지의 번호 출력 -->
	<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
		<li
			...
			<a href="/admin/ReportList?page=${idx }&&column=2">${idx }</a>
		</li>
	</c:forEach>

	<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
		<li><a href="/admin/ReportList?page=${pageMaker.endPage + 1 }&&column=2"></a></li>
	</c:if>
</ul>
